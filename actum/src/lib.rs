...
//! enums. Actum writes that for you: add `#[actum]` to an `impl` block and
...
//!   on a thread, with `#[actum(blocking)]` and no runtime at all
...
//! #[tokio::main]
//! async fn main() {
//!     // An actum handle is created and initialized with the Greeter struct
...
//! ```
//! # use actum::actum;
//! # use actum::__private::{Actor, Reply, next, reply};
//! # struct Greeter {}
//! impl Greeter {
//!     fn say_hi(&self, name: String) -> String {
//!         format!("hi {}", name)
...
//! where
//!     R: actum::Stream<Item = GreeterCall> + Unpin,
//! {
...
//! pub struct GreeterHandle(actum::Handle<Greeter, GreeterCall>);
//!
//! impl GreeterHandle {
//!     pub fn new(val: Greeter) -> Self {
//!         GreeterHandle(actum::__private::spawn(val, run_greeter))
//!     }
//!
...
//! half any [`Stream`] of it, which actum re-exports so that naming them
//! needs no dependency on the futures crates. There is no trait of actum's
//! own in between.
//!
...
//!
//! `#[actum(blocking)]` generates the same three things with `async` taken out
...
//!
//! Every method in an `#[actum]` block gets a handle method. For an inherent
...
//! [`#[actum::skip]`](macro@crate::skip) instead:
//!
//! ```
//! # use actum::actum;
//! # #[derive(Clone, Debug)]
...
//!
//! Each `#[actum]` block generates a handle struct named `{Type}Handle` and a
...
//!
//! Actum ships handles for common standard library types, generated from an
//! `#[actum]` block the same way your own are:
//!
...
// Generated code refers to ::actum, which has to resolve inside this crate too.
extern crate self as actum;
...
pub use actum_macros::{actum, skip};
pub use extensions::{
...
/// The crate's own items that the [`actum`](macro@crate::actum) macro needs in
